...
s.t. LLAMADAS_MAXIMAS {i in LOCALIZACIONES}: sum{j in DISTRITOS} llamadas[i,j] <= llamadas_maximas;
...
s.t. BALANCE_ESFUERZO {i in LOCALIZACIONES, a in LOCALIZACIONES: a!=i}: sum{j in DISTRITOS} llamadas[i,j] <= (1 + porcentaje_balance)*sum{j in DISTRITOS}llamadas[a, j];
